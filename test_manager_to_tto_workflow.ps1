...
try {
    ...
    Write-Host "✅ Manager requests endpoint working: $($managerRequests.Count) requests found" -ForegroundColor Green
    ...
    if ($pendingRequest) {
        ...
        Write-Host "⚠️ No pending requests found - testing with any available request" -ForegroundColor Yellow
...
try {
    ...
    if ($approvalResponse.status -eq "MANAGER_APPROVED") {
        Write-Host "✅ Status correctly updated to MANAGER_APPROVED" -ForegroundColor Green
        Write-Host "📧 TTO email notification should have been sent automatically!" -ForegroundColor Yellow
    } else {
...
try {
    ...
    Write-Host "✅ TTO requests endpoint working: $($ttoRequests.Count) total requests" -ForegroundColor Green
    ...
    if ($ourRequest) {
        Write-Host "✅ Test request $testRequestId is visible in TTO dashboard" -ForegroundColor Green
        Write-Host "   - Status: $($ourRequest.status)" -ForegroundColor Cyan
    } else {
        Write-Host "⚠️ Test request not found in TTO dashboard (may need refresh)" -ForegroundColor Yellow
    }
} catch {
...
try {
    $ttoDashboard = Invoke-RestMethod -Uri "$baseUrl/tto/requests/fast?page=0&size=10" -Method GET
    Write-Host "✅ TTO Dashboard Fast Load: $($ttoDashboard.totalElements) requests" -ForegroundColor Green
...
Write-Host "✅ Manager Approval Endpoint: Working" -ForegroundColor Green
Write-Host "✅ TTO Email Notification: Configured (check email)" -ForegroundColor Green
Write-Host "✅ TTO Dashboard Access: Working" -ForegroundColor Green
Write-Host "✅ Request Status Update: Working" -ForegroundColor Green
Write-Host "===========================================" -ForegroundColor Cyan

Write-Host "`n🎉 Manager to TTO Email Workflow Test Complete!" -ForegroundColor Green
...
Write-Host "`n📧 Check TTO email inbox for the notification!" -ForegroundColor Yellow
