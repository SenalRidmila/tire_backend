...
try {
    ...
    Write-Host "Success: Test request created successfully!" -ForegroundColor Green
...
# Test 2: Manager approves the request (this should trigger TTO email)
Write-Host "`n2. Manager approving request (should trigger TTO email)..." -ForegroundColor Green
try {
    ...
    if ($approvalResponse.status -eq "MANAGER_APPROVED") {
        Write-Host "EMAIL NOTIFICATION SENT TO TTO AUTOMATICALLY!" -ForegroundColor Yellow -BackgroundColor Blue
...
Write-Host "`n3. Verifying TTO dashboard shows the approved request..." -ForegroundColor Green
try {
    ...
    if ($approvedRequest) {
        Write-Host "Success: Approved request visible in TTO dashboard" -ForegroundColor Green
        Write-Host "   - Status: $($approvedRequest.status)" -ForegroundColor Cyan
        Write-Host "   - Ready for TTO action" -ForegroundColor Cyan
    } else {
        Write-Host "Warning: Approved request not found in TTO dashboard" -ForegroundColor Yellow
    }
...
# Test 4: Test TTO dashboard URL (simulating email click)
Write-Host "`n4. Testing TTO dashboard URL (simulating email click)..." -ForegroundColor Green
try {
    ...
    Write-Host "Success: TTO dashboard backend API working" -ForegroundColor Green
...
Write-Host "`nCHECK TTO EMAIL INBOX for the notification!" -ForegroundColor Yellow -BackgroundColor Blue
